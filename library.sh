...
# Copyright 2018 The Khulnasoft Authors
#
...
readonly KHULNASOFT_TESTS_PROJECT=khulnasoft-tests
...
function __resolveRepoName() {
  ...
  repoName="${repoName#khulnasoft-extensions-}" # Remove khulnasoft-extensions- prefix if any
  repoName="${repoName#khulnasoft-}" # Remove khulnasoft- prefix if any
  echo "${repoName}"
}
...
export TMPDIR="${TMPDIR:-$(mktemp -u -t -d khulnasoft.XXXXXXXX)}"
mkdir -p "$TMPDIR"
...
# Install Khulnasoft Serving in the current cluster.
# Parameters: $1 - Khulnasoft Serving crds manifest.
#             $2 - Khulnasoft Serving core manifest.
#             $3 - Khulnasoft net-istio manifest.
function start_khulnasoft_serving() {
  header "Starting Khulnasoft Serving"
  subheader "Installing Khulnasoft Serving"
  ...
  kubectl apply -f "$3"
  wait_until_pods_running khulnasoft-serving || return 1
}

# Install the stable release Khulnasoft/serving in the current cluster.
# Parameters: $1 - Khulnasoft Serving version number, e.g. 0.6.0.
function start_release_khulnasoft_serving() {
  start_khulnasoft_serving "https://storage.googleapis.com/khulnasoft-releases/serving/previous/v$1/serving-crds.yaml" \
    "https://storage.googleapis.com/khulnasoft-releases/serving/previous/v$1/serving-core.yaml" \
    "https://storage.googleapis.com/khulnasoft-releases/net-istio/previous/v$1/net-istio.yaml"
}

# Install the latest stable Khulnasoft Serving in the current cluster.
function start_latest_khulnasoft_serving() {
  start_khulnasoft_serving "${KHULNASOFT_SERVING_RELEASE_CRDS}" "${KHULNASOFT_SERVING_RELEASE_CORE}" "${KHULNASOFT_NET_ISTIO_RELEASE}"
}

# Install Khulnasoft Eventing in the current cluster.
# Parameters: $1 - Khulnasoft Eventing manifest.
function start_khulnasoft_eventing() {
  header "Starting Khulnasoft Eventing"
  subheader "Installing Khulnasoft Eventing"
  echo "Installing Eventing CRDs from $1"
  kubectl apply --selector khulnasoft.dev/crd-install=true -f "$1"
  echo "Installing the rest of eventing components from $1"
  kubectl apply -f "$1"
  wait_until_pods_running khulnasoft-eventing || return 1
}

# Install the stable release Khulnasoft/eventing in the current cluster.
# Parameters: $1 - Khulnasoft Eventing version number, e.g. 0.6.0.
function start_release_khulnasoft_eventing() {
  start_khulnasoft_eventing "https://storage.googleapis.com/khulnasoft-releases/eventing/previous/v$1/eventing.yaml"
}

# Install the latest stable Khulnasoft Eventing in the current cluster.
function start_latest_khulnasoft_eventing() {
  start_khulnasoft_eventing "${KHULNASOFT_EVENTING_RELEASE}"
}

# Install Khulnasoft Eventing extension in the current cluster.
# Parameters: $1 - Khulnasoft Eventing extension manifest.
#             $2 - Namespace to look for ready pods into
function start_khulnasoft_eventing_extension() {
  header "Starting Khulnasoft Eventing Extension"
...
function go_run() {
  ...
  if [[ "$package" == *@latest ]] && [[ "$package" != khulnasoft.dev* ]]; then
...
function foreach_go_module() {
  ...
  done < <(go_run khulnasoft.dev/toolbox/modscope@latest ls -p)
}
...
#                         Khulnasoft components. ex: --release v0.18. Defaults to
#                         "main".
...
function __go_update_deps_for_module() {
  ...
  export GOFLAGS=""
  export GONOSUMDB="${GONOSUMDB:-},khulnasoft.dev/*"
  export GONOPROXY="${GONOPROXY:-},khulnasoft.dev/*"
  ...
  if [[ $UPGRADE == 1 ]]; then
    ...
    FLOATING_DEPS+=( $(go_run khulnasoft.dev/toolbox/buoy@latest float ./go.mod "${buoyArgs[@]}") )
...
function go_mod_module_name() {
  go_run khulnasoft.dev/toolbox/modscope@latest current
}
...
function run_kntest() {
  go_run khulnasoft.dev/test-infra/tools/kntest/cmd/kntest@latest "$@"
}
...
# Return whether the given parameter is the khulnasoft release/nightly GCF.
# Parameters: $1 - full GCR name, e.g. gcr.io/khulnasoft-foo-bar
function is_protected_gcr() {
  [[ -n $1 && $1 =~ ^gcr.io/khulnasoft-(releases|nightly)/?$ ]]
}

# Return whether the given parameter is any cluster under ${KHULNASOFT_TESTS_PROJECT}.
# Parameters: $1 - Kubernetes cluster context (output of kubectl config current-context)
function is_protected_cluster() {
  # Example: gke_khulnasoft-tests_us-central1-f_prow
  [[ -n $1 && $1 =~ ^gke_${KHULNASOFT_TESTS_PROJECT}_us\-[a-zA-Z0-9]+\-[a-z]+_[a-z0-9\-]+$ ]]
}

# Return whether the given parameter is ${KHULNASOFT_TESTS_PROJECT}.
# Parameters: $1 - project name
function is_protected_project() {
  [[ -n $1 && "$1" == "${KHULNASOFT_TESTS_PROJECT}" ]]
}
...
function remove_broken_symlinks() {
  for link in $(find "$1" -type l); do
    ...
    if [[ ${target} != *github.com/khulnasoft/* && ${target} != *khulnasoft.dev/* ]]; then
      unlink "${link}"
      continue
    fi
  done
...
# Returns the URL to the latest manifest for the given Khulnasoft project.
# Parameters: $1 - repository name of the given project
#             $2 - name of the yaml file, without extension
function get_latest_khulnasoft_yaml_source() {
  local repo_name="$1"
  local yaml_name="$2"
  ...
  if is_release_branch; then
    ...
    local yaml_source_path="$(
      gsutil ls "gs://khulnasoft-releases/${repo_name}/previous/v${major_minor}.*/${yaml_name}.yaml" 2> /dev/null \
      | sort \
      | tail -n 1 \
      | cut -b6-)"
  ...
  echo "https://storage.googleapis.com/khulnasoft-nightly/${repo_name}/latest/${yaml_name}.yaml"
}
...
function latest_version() {
  ...
  if [[ "$branch_name" == "release-"* ]]; then
    ...
    # Nearest tag with the `khulnasoft-` prefix
    local tag=$(git describe --abbrev=0 --match "khulnasoft-v[0-9]*")
...
readonly REPO_NAME_FORMATTED="Khulnasoft $(capitalize "${REPO_NAME//-/ }")"

# Public latest nightly or release yaml files.
readonly KHULNASOFT_SERVING_RELEASE_CRDS="$(get_latest_khulnasoft_yaml_source "serving" "serving-crds")"
readonly KHULNASOFT_SERVING_RELEASE_CORE="$(get_latest_khulnasoft_yaml_source "serving" "serving-core")"
readonly KHULNASOFT_NET_ISTIO_RELEASE="$(get_latest_khulnasoft_yaml_source "net-istio" "net-istio")"
readonly KHULNASOFT_EVENTING_RELEASE="$(get_latest_khulnasoft_yaml_source "eventing" "eventing")"
readonly KHULNASOFT_EVENTING_RELEASE_TLS="$(get_latest_khulnasoft_yaml_source "eventing" "eventing-tls-networking")"
