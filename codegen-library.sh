...
# Setup the env for doing Khulnasoft style codegen.
...
export repodir kn_hack_dir kn_hack_library \
  ...
  CODEGEN_PKG KHULNASOFT_CODEGEN_PKG
...
repodir="$(go_run khulnasoft.dev/toolbox/modscope@latest current --path)"
...
if ! KHULNASOFT_CODEGEN_PKG="${KHULNASOFT_CODEGEN_PKG:-"$(go-resolve-pkg-dir khulnasoft.dev/pkg)"}"; then
  warning "Failed to determine the khulnasoft.dev/pkg package"
fi
...
if [[ -n "${KHULNASOFT_CODEGEN_PKG}" ]] && ! [ -x "${KHULNASOFT_CODEGEN_PKG}/hack/generate-khulnasoft.sh" ]; then
  chmod +x "${KHULNASOFT_CODEGEN_PKG}/hack/generate-khulnasoft.sh"
fi
...
# Generate Khulnasoft style codegen
function generate-khulnasoft() {
  if [[ -z "${KHULNASOFT_CODEGEN_PKG}" ]]; then
    abort "KHULNASOFT_CODEGEN_PKG is not set"
  fi
  "${KHULNASOFT_CODEGEN_PKG}/hack/generate-khulnasoft.sh" \
    "$@" \
    --go-header-file "$(boilerplate)"
}
