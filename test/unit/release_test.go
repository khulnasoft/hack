...
func TestBuildFromSource(t *testing.T) {
	...
	outChecks := []check{
		contains("Signing Images with the identity signer@khulnasoft-releases.test"),
		contains("Signing checksums with the identity signer@khulnasoft-releases.test"),
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			sc := testReleaseShellScript(
				envs(map[string]string{
					...
					"SIGNING_IDENTITY":             "signer@khulnasoft-releases.test",
				}),
...
func TestReleaseFlagParsingDefaults(t *testing.T) {
	...
	tcs := []testCase{{
		...
		stdout: lines(
			":gcr.io/khulnasoft-nightly:",
			":khulnasoft-nightly/hack::",
		),
	}, {
