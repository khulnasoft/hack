...
export INGRESS_CLASS=${INGRESS_CLASS:-istio.ingress.networking.khulnasoft.dev}

function is_ingress_class() {
  [[ "${INGRESS_CLASS}" == *"${1}"* ]]
}

# Copied from https://github.com/khulnasoft/client/blob/main/test/common.sh#L32
function install_istio() {
  ...
  local LATEST_NET_ISTIO_RELEASE_VERSION=$(curl -L --silent "https://api.github.com/repos/khulnasoft/net-istio/releases" | \
    jq -r '[.[].tag_name] | sort_by( sub("khulnasoft-";"") | sub("v";"") | split(".") | map(tonumber) ) | reverse[0]')
  # And checkout the setup script based on that release
  local NET_ISTIO_DIR=$(mktemp -d)
  (
    cd $NET_ISTIO_DIR \
      && git init \
      && git remote add origin https://github.com/khulnasoft-extensions/net-istio.git \
