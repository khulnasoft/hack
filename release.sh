...
# This is a helper script for Khulnasoft release scripts.
# See README.md for instructions on how to use it.

source "$(dirname "${BASH_SOURCE[0]}")/library.sh"

# Organization name in GitHub; defaults to Khulnasoft.
readonly ORG_NAME="${ORG_NAME:-khulnasoft}"
...
function main() {
  ...
  if [[ -n "${RELEASE_BRANCH}" && -z "${FROM_NIGHTLY_RELEASE}" && "${current_branch}" != "${RELEASE_BRANCH}" ]]; then
    ...
    # HACK HACK HACK
    # Rerun the release script from the release branch. Fixes https://github.com/khulnasoft/test-infra/issues/1262
    ./hack/release.sh "$@"
    exit "$?"
  fi
...
function publish_to_github() {
  ...
  # v1.1.1 = v0.28.1
  #
  # See: https://github.com/khulnasoft/hack/pull/97
  if [[ "$TAG" == "v1"* ]]; then
