...
# This is a helper script for Khulnasoft presubmit test scripts.
...
function __build_test_runner_for_module() {
  ...
  local tags
  tags="$(go run khulnasoft.dev/toolbox/go-ls-tags@latest --joiner=,)"
  local go_pkg_dirs
...
function main() {
  ...
  if (( IS_PROW )); then
    ...
    [[ -f /commit_hash ]] && echo "Prow test image was built from $(cat /commit_hash) commit which is viewable at https://github.com/khulnasoft/test-infra/tree/$(cat /commit_hash) " || echo "unknown"
  fi
